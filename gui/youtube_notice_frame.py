...
class YouTubeNoticeFrame(ctk.CTkFrame):
    def __init__(self, master=None):
        ...
        ctk.CTkSwitch(self, text="YouTubeLive：終了通知を送信する", variable=self.var_offline, font=DEFAULT_FONT, onvalue=True, offvalue=False).grid(row=1, column=0, sticky="w", pady=5)
        ctk.CTkSwitch(self, text="YouTube動画：動画投稿通知を送信する", variable=self.var_newvideo, font=DEFAULT_FONT, onvalue=True, offvalue=False).grid(row=2, column=0, sticky="w", pady=5)
        # テンプレート選択（開始）
        ...
        self.img_preview.grid(row=10, column=0, rowspan=2, sticky="w", padx=(0, 10), pady=(0, 10))
        self.lbl_img = ctk.CTkLabel(self, text=os.path.basename(self.img_path.get()), font=DEFAULT_FONT)
        self.lbl_img.grid(row=10, column=1, sticky="w", pady=(0, 5))
        ctk.CTkButton(self, text="画像変更...", command=self.change_image_file, font=DEFAULT_FONT, width=140).grid(row=11, column=1, sticky="w", pady=(0, 10))
        ctk.CTkButton(self, text="保存", command=self.save_yt_settings, font=DEFAULT_FONT, width=140).grid(row=12, column=1, sticky="w", pady=(10, 0))
        self.status_label = ctk.CTkLabel(self, text="", font=DEFAULT_FONT)
        self.status_label.grid(row=13, column=0, columnspan=2, sticky="w", pady=(5, 0))
