...
def create_webhookurl_tab(parent):
    ...
    # 一時URLは毎回最新値を取得するようにStringVarで管理
    import threading
    import time
    temporary_url_var = ctk.StringVar(value=os.getenv('WEBHOOK_CALLBACK_URL_TEMPORARY', ''))
    def update_temp_url_periodically():
                ...
                with open(env_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        if line.startswith('WEBHOOK_CALLBACK_URL_TEMPORARY='):
                            ...
                            break
            if url != last_url:
                temporary_url_var.set(url)
                last_url = url
            time.sleep(2)  # 2秒ごとに監視
    threading.Thread(target=update_temp_url_periodically, daemon=True).start()
    # 恒久用
    ...
    entry_temp = ctk.CTkEntry(frame, font=DEFAULT_FONT, textvariable=temporary_url_var)
    ...
    def save_perm_url():
        ...
        show_ctk_info(frame, '保存完了', 'WEBHOOK_CALLBACK_URL_PERMANENTを保存しました。')
