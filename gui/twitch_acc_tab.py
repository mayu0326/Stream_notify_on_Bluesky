    ...
    def __init__(self, master=None):
        ...
        def save_twitch_account():
            ...
            if not cid or not csecret or not user_input:
                ...
                show_ctk_error(self, "エラー", "全ての項目を入力してください。")
                return
            def is_numeric(s):
                return s.isdigit()
            ...
            try:
                if not is_numeric(user_input):
                    try:
                        ...
                        from gui.app_gui import show_ctk_error
                        show_ctk_error(self, "変換エラー", f"Twitchユーザー名からID変換に失敗しました: {e}")
                        return
            except Exception as e:
                self.status_label.configure(text=f"Twitch API通信失敗: {e}", font=DESC_FONT, text_color="red")
                from gui.app_gui import show_ctk_error
                show_ctk_error(self, "通信エラー", f"Twitch API通信に失敗: {e}")
                return
            ...
            try:
                ...
                from gui.app_gui import show_ctk_error
                show_ctk_error(self, '保存エラー', f'Twitch設定の保存に失敗しました: {e}')
            ...
            show_ctk_info(self, "保存完了", "Twitchアカウント情報を保存しました。")
