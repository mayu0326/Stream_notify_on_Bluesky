...
def get_current_time():
    # 現在時刻をタイムゾーン付きで取得するユーティリティ関数。EventSubのタイムスタンプ生成等で他モジュールからも利用されるため削除不可。
...
def create_eventsub_subscription(event_type: str, logger_to_use=None, webhook_url=None):
    ...
    current_logger.error(f"EventSubリクエスト: url={url} headers={headers} payload={payload}")
    ...
    try:
        ...
        current_audit_logger.warning(
            f"EventSubサブスクリプション作成: {event_type} 失敗: {reason} 詳細: {error_json}")
        current_logger.error(
            f"EventSubサブスクリプション ({event_type}) 作成失敗: {reason} 詳細: {error_json}")
